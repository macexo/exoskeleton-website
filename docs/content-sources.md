...
The Design page uses a team-supplied photograph with four selectable callouts for visible hardware areas and a sensing → prediction → control → actuation loop with movement feedback. These are public system summaries, not a component-by-component technical annotation. Version labels are not used in the public copy. Dated competition photos elsewhere retain their own captions.
...
The team subsequently identified the Pi enclosure to the right of the pilot's hand and the leg-mounted MCU enclosures containing the motion sensors and STM32 hardware. These identifications map sensing to the controller enclosures, prediction to the onboard computer, control to the controller enclosures and visible red cable, and actuation to the hip and knee motors. Public copy retains functional descriptions rather than component specifications.

`data/suitGeometry.ts` contains manually drawn visible contours registered to the photograph, not exported CAD geometry. Separate contours stop at occlusions. Hover or keyboard focus previews the corresponding pieces; click or tap retains a selection. Control animates a trace along visible sections of the red cable, with breaks where it is hidden. The animation illustrates a connection without specifying a cable protocol or claiming a complete wiring diagram. Reduced-motion preferences disable the animation.

The selections in `data/suitParts.ts` show one short explanation at a time in a stable panel, with links to the contributing subteams. Detailed subteam responsibilities stay on the discipline pages. The selections do not assert that today's subteams had the same names when the documented suit was built, and the system names are not a component inventory or a count of physical modules. No raw geometry, dimensions, wiring diagrams or prospective design details are offered as downloads.
