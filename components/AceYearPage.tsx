...
export default function AceYearPage({ year }: { year: AceYear }) {
  ...
  return <div className="home-page inner-page">
    <PageHero eyebrow={`The competition journal / ${year.year}`} title={`ACE ${year.year}.`} accent={year.year === 2026 ? "On home ground." : "Our first steps."} image={image} imageAlt={`A McMaster pilot navigating a balance obstacle in the powered suit at ACE ${year.year}.`} focal="52% 50%" imageCaption={`${year.host} / ${year.note}`} actions={<><a href="#gallery" className="home-button">See the season <ArrowRight /></a><Link href="/design" className="home-text-link">Inside the engineering <ArrowRight /></Link></>} below={<><div className="competition-facts"><div className="home-container"><div className="competition-placing"><strong>{year.placement}</strong><span>Overall finish</span></div><div><p className="eyebrow">Host university</p><strong>{year.hostLong}</strong></div><div><p className="eyebrow">The challenge</p><strong>{ACE_EVENTS.length} events. One suit.</strong></div></div></div><PageNav items={[{href:"#challenge",label:"The competition"},{href:"#gallery",label:"Photo journal"},{href:"#seasons",label:"Other seasons"}]} /></>}>
      {year.summary}
    </PageHero>
